...
def get_history(biz, uin, key, cmp_date="2023-5-31", idx=0, offset=0, count=10, continue_flag=True, max=10):
    ...
    if js.get('can_msg_continue') and continue_flag and (offset < max):
        ...
        time.sleep(10)  # 防封
        get_history(biz, uin, key, cmp_date, idx, offset, 10, True) # 测试一次就不继续
    else:
        print('结束，没有更多文章。')


def get_art_info(json_art_info, cmp_date, idx):
